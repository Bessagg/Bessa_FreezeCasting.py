...
df.drop(['last_updated_x', 'name_author', 'authors', 'author_ID', 'paper_ID', 'sample_ID',
...
# 'technique', 'direction' only have 600 rows, so where dropped
selected_cols = ['pore_structure', 'name_part1', 'name_fluid1', 'sublimated', 'material',
                 'material_group',
...
# plot porosity agasint string columns
string_cols = ['pore_structure', 'name_part1', 'name_fluid1', 'sublimated', 'material',
               'material_group']
...
for col in string_cols:
    ...
    plt.title(col + " vs " + 'porosity')
    plt.scatter(filtered_df['porosity'], filtered_df[col])
...
#     filtered_df = df[df[col].notnull()]
#     n_rank = 5
#     top_n = filtered_df[col].value_counts()[:n_rank].sort_values(ascending=False)
#     plt.title(col + " vs " + 'porosity')
#     categories = top_n.axes[0]
...
#         # plt.hist(filtered_df['porosity'], 10, density=True, linewidth=2, edgecolor='k')
#         # sns.distplot(filtered_df['porosity'], fit=norm, kde=False, color=cmap[index], label=category)
#         # hist_kws = {"histtype": "step", "linewidth": 3,
#         #             "alpha": 1, "color": "g"}
#         # sns.distplot(filtered_df['porosity'], color=cmap[index], kde=False,
#         #                   fit=stats.gamma, hist=None, label=category)
#
#         sns.histplot(filtered_df['porosity'], kde=True, stat="density", linewidth=1, label=category, color=cmap[index],
#                      element="step", fill=False, cbar=True)
#     plt.legend()
#     plt.show()
# plt.close("all")
